...
def upgrade() -> None:
    ...
    # Pattern: update data -> rename old type -> create new type -> alter columns -> drop old type
    ...
    # =========================================================================
    op.execute("""
        UPDATE moderation_states
        SET visibility = LOWER(visibility::text)::moderationvisibility
    """)
    op.execute("""
        UPDATE moderation_log
        SET new_visibility = LOWER(new_visibility::text)::moderationvisibility
        WHERE new_visibility IS NOT NULL
    """)
    ...
    op.execute("""
        ...
        USING visibility::text::moderationvisibility
    """)
    op.execute("""
        ALTER TABLE moderation_log
        ALTER COLUMN new_visibility TYPE moderationvisibility
        USING new_visibility::text::moderationvisibility
    """)
    ...
    op.execute("""
        ...
        SET trigger = LOWER(trigger::text)::moderationtrigger
    """)
    ...
    op.execute("""
        ...
        USING trigger::text::moderationtrigger
    """)
    ...
    op.execute("""
        UPDATE moderation_log
        SET action = LOWER(action::text)::moderationaction
    """)
    ...
    op.execute("""
        UPDATE moderation_states
        SET object_type = LOWER(object_type::text)::moderationobjecttype
    """)
...
def downgrade() -> None:
    ...
    op.execute("""
        UPDATE moderation_states
        SET object_type = UPPER(object_type::text)::moderationobjecttype
    """)
    op.execute("ALTER TYPE moderationobjecttype RENAME TO moderationobjecttype_old")
    op.execute("CREATE TYPE moderationobjecttype AS ENUM ('HOST_REQUEST', 'GROUP_CHAT')")
    op.execute("""
    ...
    op.execute("""
        UPDATE moderation_log
        SET action = UPPER(action::text)::moderationaction
    """)
    op.execute("ALTER TYPE moderationaction RENAME TO moderationaction_old")
    op.execute("CREATE TYPE moderationaction AS ENUM ('CREATE', 'APPROVE', 'HIDE', 'FLAG', 'UNFLAG')")
    op.execute("""
    ...
    op.execute("""
        UPDATE moderation_queue
        SET trigger = UPPER(trigger::text)::moderationtrigger
    """)
    op.execute("ALTER TYPE moderationtrigger RENAME TO moderationtrigger_old")
    op.execute(
        "CREATE TYPE moderationtrigger AS ENUM ('INITIAL_REVIEW', 'USER_FLAG', 'MACHINE_FLAG', 'MODERATOR_REVIEW')"
    ...
    op.execute("""
        UPDATE moderation_states
        SET visibility = UPPER(visibility::text)::moderationvisibility
    """)
    op.execute("""
        UPDATE moderation_log
        SET new_visibility = UPPER(new_visibility::text)::moderationvisibility
        WHERE new_visibility IS NOT NULL
    """)
    op.execute("ALTER TYPE moderationvisibility RENAME TO moderationvisibility_old")
    op.execute("CREATE TYPE moderationvisibility AS ENUM ('HIDDEN', 'SHADOWED', 'UNLISTED', 'VISIBLE')")
